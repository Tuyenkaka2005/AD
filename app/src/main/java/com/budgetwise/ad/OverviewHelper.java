...
public class OverviewHelper {
    ...
    public static MonthlySummary getMonthlySummary(Context context, int month, int year) {
        ...
        if (userId == null) {
            // Nếu chưa đăng nhập → trả về dữ liệu rỗng (tránh crash)
            return new MonthlySummary(0, 0, 0, new ArrayList<>());
        }
        ...
        String sqlCat = "SELECT c." + DatabaseContract.CategoryEntry.COLUMN_NAME + ", " +
                ...
                "LEFT JOIN " + DatabaseContract.ExpenseEntry.TABLE_NAME + " e " +
                "   ON c." + DatabaseContract.CategoryEntry.COLUMN_CATEGORY_ID + " = e." + DatabaseContract.ExpenseEntry.COLUMN_CATEGORY_ID +
                "   AND e." + DatabaseContract.ExpenseEntry.COLUMN_DATE + " BETWEEN ? AND ? " +
                "   AND e." + DatabaseContract.ExpenseEntry.COLUMN_USER_ID + "=? " +
                "LEFT JOIN " + DatabaseContract.BudgetEntry.TABLE_NAME + " b " +
                "   ON c." + DatabaseContract.CategoryEntry.COLUMN_CATEGORY_ID + " = b." + DatabaseContract.BudgetEntry.COLUMN_CATEGORY_ID +
                "   AND b." + DatabaseContract.BudgetEntry.COLUMN_MONTH + "=? " +
                "   AND b." + DatabaseContract.BudgetEntry.COLUMN_YEAR + "=? " +
                "   AND b." + DatabaseContract.BudgetEntry.COLUMN_IS_ACTIVE + "=1 " +
                ...
                " ORDER BY spent DESC";  // Sắp xếp theo chi tiêu nhiều nhất
        ...
        while (c.moveToNext()) {
            ...
            double percent = budget > 0 ? (spent / budget) * 100.0 : 0;
